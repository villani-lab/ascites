...
label_gene_list <- list("cd4" = c("RPL27A", "JUN", "CX3CR1", "PRF1", "GNLY", "CD40LG", "LAPTM5", "CD44", "ZNF683", "ARID5A", "CXCR3", "IL10RA", "HLA-DRA", "TGFB1"),
                        "cd8" = c("RPL27A","TOX", "TCF7", "MYC", "CXCR6", "CXCR3", "CCR7", "GZMK", "ITM2C", "CX3CR1", "GZMA", "IFIT3", "CD70", "PLEK"))
...
for (lin in c("cd4", "cd8")){
  ...
  p <- ggplot(lin_data, aes(x = log2FoldChange, y = -log10(pvalue))) +
    ...
    geom_label_repel(data = lin_data %>% dplyr::filter(gene %in% label_up), aes(label = gene), max.overlaps = 20, color = "blue", size = 6, fontface = "italic") +
    geom_label_repel(data = lin_data %>% dplyr::filter(gene %in% label_down), aes(label = gene), max.overlaps = 20, color = "red", size = 6, fontface = "italic") +
...
ggsave("/projects/home/nealpsmith/projects/ascites/figures/resubmission/fig_panels/fig_2e_g.pdf",
...
for (lin in c("cd8", "cd4")){
  ...
  for (gene in genes){
    ...
    p <- ggplot(plot_df, aes(x = tissue_type, y = !!as.symbol(gene), group = TRB_cdr3, color = paper_id)) +
      geom_point(size = 3) +
    ...
    supp <- plot_df %>% dplyr::select(!!as.symbol(gene), paper_id, tissue_type, TRB_cdr3) %>%
      rename( "expression" = !!as.symbol(gene)) %>% mutate(gene = gene, lineage = lin)
    supp_table_df <- rbind(supp_table_df, supp)
...
write.csv(supp_table_df,
          "/projects/home/nealpsmith/projects/ascites/data/plot_data/fig_2_f_h.csv",
          row.names = FALSE)
